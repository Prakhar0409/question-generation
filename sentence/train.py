...
def load_fields(dataset, data_type, checkpoint):
    ...
    if data_type == 'text':
        ...
        print(' * vocabulary type. source = %s; target = %s' %
              (type(fields['src'].vocab), type(fields['tgt'].vocab)))
    else:
...
def main():
    ...
    print('loading datasets lazily')
    first_dataset = next(lazily_load_dataset("train"))
    data_type = first_dataset.data_type

    # Load fields generated from preprocess phase.
    print('loading fields from preprocess phase - Vocabulary')
    ...
    # Build model.
    model = {}
    if checkpoint is not None and checkpoint['models']:
        model['encoder'] = onmt.Models.loadEncoder(checkpoint['models']['encoder'])
        model['decoder'] = onmt.Models.loadEncoder(checkpoint['models']['decoder'])
    else: 
        enc_embeddings = onmt.ModelConstructor.make_embeddings(opt, fields['src'].vocab, [], for_encoder=True)
        dec_embeddings = onmt.ModelConstructor.make_embeddings(opt, fields['tgt'].vocab, [], for_encoder=False)
        model['encoder'] = onmt.ModelConstructor.make_encoder(opt, enc_embeddings)
        model['decoder'] = onmt.ModelConstructor.make_decoder(opt, dec_embeddings)
